...
/obj/machinery/computer/shuttle/Destroy()
	. = ..()
	SSorbits.open_orbital_maps -= SStgui.close_uis(src)
	shuttleObject = null
	//De-link the port
	if(my_port)
...
/obj/machinery/computer/shuttle/examine(mob/user)
	. = ..()
...
/obj/machinery/computer/shuttle/ui_close(mob/user, datum/tgui/tgui)
	SSorbits.open_orbital_maps -= tgui

/obj/machinery/computer/shuttle/ui_static_data(mob/user)
	var/list/data = list()
		...
		for(var/obj/docking_port/stationary/stationary_port as() in SSshuttle.stationary)
			if(LAZYLEN(shuttleObject.docking_target.linked_z_level))
				for(var/datum/space_level/level in shuttleObject.docking_target.linked_z_level)
					if(stationary_port.z == level.z_value && (stationary_port.shuttle_id in valid_docks))
						data["validDockingPorts"] += list(list(
							"name" = stationary_port.name,
							"id" = stationary_port.shuttle_id,
						))
	return data
...
/obj/machinery/computer/shuttle/ui_act(action, params)
	...
	switch(action)
		...
		if("gotoPort")
			...
			if(!target_port)
				return
			if(!(target_port.shuttle_id in valid_docks))
				log_admin("[usr] attempted to forge a target location through a tgui exploit on [src]")
				message_admins("[ADMIN_FULLMONTY(usr)] attempted to forge a target location through a tgui exploit on [src]")
				return
			...
			switch(SSshuttle.moveShuttle(shuttleId, target_port.shuttle_id, 1))
				if(0)
...
/obj/machinery/computer/shuttle/proc/launch_shuttle()
...
/obj/machinery/computer/shuttle/proc/random_drop(datum/orbital_object/shuttle/_shuttleObject = shuttleObject, _shuttleId = shuttleId)
	...
	random_port.shuttle_id = "randomdroplocation_[random_drops++]"
	...
	while(sanity > 0)
		...
		//Ok lets go there
		switch(SSshuttle.moveShuttle(_shuttleId, random_port.shuttle_id, 1))
			if(0)
				say("Initiating supercruise throttle-down, prepare for landing.")
				if(current_user)
					remove_eye_control(current_user)
...
/obj/machinery/computer/shuttle/proc/unfreeze_shuttle(obj/docking_port/mobile/shuttle_dock, datum/space_level/target_spacelevel)
...
/obj/machinery/computer/shuttle/allowed(mob/M)

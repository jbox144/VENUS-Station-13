...
/obj/machinery/computer/shuttle/Initialize(mapload, obj/item/circuitboard/C)
	. = ..()
...
/obj/machinery/computer/shuttle/proc/GrantActions(mob/living/user)
	if(off_action)
...
/obj/machinery/computer/shuttle/proc/CreateEye()
...
/obj/machinery/computer/shuttle/proc/give_eye_control(mob/user)
	if(!isliving(user))
		return
	if(!eyeobj)
		CreateEye()
...
/obj/machinery/computer/shuttle/remove_eye_control(mob/user)
	if(!user)
		return
...
/obj/machinery/computer/shuttle/proc/placeLandingSpot()
	...
	if(designate_time && (landing_clear != SHUTTLE_DOCKER_BLOCKED))
		...
		var/wait_completed = do_after(current_user, designate_time, designating_target_loc, TRUE, TRUE, CALLBACK(src, .proc/canDesignateTarget))
...
/obj/machinery/computer/shuttle/proc/canDesignateTarget()
	if(!designating_target_loc || !current_user || (eyeobj.loc != designating_target_loc) || (machine_stat & (NOPOWER|BROKEN)) )
		return FALSE
	return TRUE

/obj/machinery/computer/shuttle/proc/rotateLandingSpot()
	var/mob/camera/aiEye/remote/shuttle_docker/the_eye = eyeobj
...
/obj/machinery/computer/shuttle/proc/checkLandingSpot()
	var/mob/camera/aiEye/remote/shuttle_docker/the_eye = eyeobj
...
/obj/machinery/computer/shuttle/proc/checkLandingTurf(turf/T, list/overlappers)
...
/obj/machinery/computer/shuttle/proc/update_hidden_docking_ports(list/remove_images, list/add_images)
...
/mob/camera/aiEye/remote/shuttle_docker/setLoc(T)
	..()
	var/obj/machinery/computer/shuttle/console = origin
...
/datum/action/innate/shuttledocker_rotate/Activate()
	...
	var/mob/camera/aiEye/remote/shuttle_docker/remote_eye = C.remote_control
	var/obj/machinery/computer/shuttle/origin = remote_eye.origin
...
/datum/action/innate/camera_jump/shuttle_docker/Activate()
	...
	var/mob/living/C = target
	var/mob/camera/aiEye/remote/shuttle_docker/remote_eye = C.remote_control
	var/obj/machinery/computer/shuttle/console = remote_eye.origin
	...
	for(var/V in SSshuttle.stationary)
		...
		if(console.shuttleObject.docking_target.z_in_contents(S.z) && (S.shuttle_id in console.valid_docks))
			L["(L.len)[S.name]"] = S
	...
	if(selected)
		...
		if(T)
			...
			C.overlay_fullscreen("flash", /atom/movable/screen/flash)

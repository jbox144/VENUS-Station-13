...
/datum/orbital_objective/vip_recovery/generate_objective_stuff(turf/chosen_turf)
	...
	var/list/mob/candidates = pollGhostCandidates("Do you want to play as a survivalist?", ROLE_SURVIVALIST)
	if(LAZYLEN(candidates))
		var/mob/dead/observer/G = pick(candidates)
		G.transfer_ckey(created_human, TRUE)
	...
	switch(pickweight(list("centcom_official" = 4, "greytide" = 3)))
		if("centcom_official")
			created_human.add_memory("You are a CentCom official onboard a badly damaged station. Making your way back to Space Station 13 to uncover the secrets you hold is \
		...
		if("greytide")
			created_human.add_memory("You are just an assistant on a lonely derelict station. You dream of going home, \
			...
			antag_elligable = TRUE
	created_human.mind.show_memory()
	if(antag_elligable)
		if(prob(7))

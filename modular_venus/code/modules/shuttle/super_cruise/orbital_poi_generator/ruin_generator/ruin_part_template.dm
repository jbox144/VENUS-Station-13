...
/datum/map_template/ruin_part/New(path, rename, cache)
	...
	. = ..(path, rename, TRUE)
	if(cached_map == null)
		log_mapping("Ruin part [src.mappath] doesn't exist!")
		return FALSE
	else
...
/datum/map_template/ruin_part/proc/build_coordinate(list/model, xcrd, ycrd)
	...
	while(!ispath(members[first_turf_index], /turf)) //find first /turf object in members
		first_turf_index++
		if(first_turf_index >= index) // VENUS START - Some maps are missing their turf somehow. This'll help catch 'em
			log_mapping("Ruin part [src.mappath] has no turf at [xcrd], [ycrd]!")
			return FALSE // VENUS END

...
/datum/controller/subsystem/processing/orbits/fire(resumed)
	if(resumed)
		...
		if(MC_TICK_CHECK)
			return
		//Update UIs
		for(var/datum/tgui/tgui in open_orbital_maps)
			tgui.send_update()
	...
	if(current_objective)
		if(current_objective.check_failed())
			priority_announce("Central Command priority objective failed.", "Central Command Report")
			QDEL_NULL(current_objective)
	//Process events
	for(var/datum/ruin_event/ruin_event in ruin_events)
